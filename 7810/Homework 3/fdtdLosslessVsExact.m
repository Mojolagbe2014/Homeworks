%% fdtdLosslessVsExact.m
%   Demonstrate FDTD transmission line assuming lossless
%
...
voltage = zeros(1, num+1);                                                  % initialize voltage array (numerical solution)
...
Vt_near = 0;                                                                % first terminating voltage 
% Rt_far = sqrt(L/C);
Rt_far = inf;                                                               % second terminating resistance
Vt_far = 0;                                                                 % second termination voltage
...
if(Rt_near == Inf); voltage(1) = voltage(2);   voltage_e(1) = voltage_e(2); 
else voltage(1) = Vt_near - current(1)*Rt_near;  voltage_e(1) = Vt_near - current_e(1)*Rt_near; 
end
if(Rt_far == Inf);    voltage(num+1) = voltage(num);  voltage_e(num+1) = voltage_e(num);
else voltage(num+1) = Vt_far + current(num)*Rt_far; voltage_e(num+1) = Vt_far + current_e(num)*Rt_far;
end
...
%% update interior points of exact/analytical solution
current_e(1:num) = [current_e(1), mean([current_e(1:end-1); current_e(2:end)])] - (1/Z0)*0.5*(voltage_e(2:num+1) - voltage_e(1:num));
voltage_e(2:num) = mean([voltage_e(2:num); voltage_e(3:end)]) - (Z0)*0.5*(current_e(2:num) - current_e(1:num-1));
...
while stop == 0 
    ...
    if(Rt_near == Inf)
       voltage(1) = voltage(2);   
       voltage_e(1) = voltage_e(2);  
    else
        voltage(1) = Vt_near - current(1)*Rt_near;
        voltage_e(1) = Vt_near - current_e(1)*Rt_near;
    end
    if(Rt_far == Inf)
       voltage(num+1) = voltage(num); 
       voltage_e(num+1) = voltage_e(num); 
    else
        voltage(num+1) = Vt_far + current(num)*Rt_far; 
        voltage_e(num+1) = Vt_far + current_e(num)*Rt_far; 
    end
    ...
    % update interior points of exact solution
    current_e(1:num) = current_e(1:num) - (1/Z0)*(voltage_e(2:num+1) - voltage_e(1:num));
    voltage_e(2:num) = voltage_e(2:num) - (Z0)*(current_e(2:num) - current_e(1:num-1));
