...
template<typename T> Mtx<std::complex<T>>::Mtx(int n, int m, std::complex<T>** dbp) { // constructor
    nrows = n;
    ncols = m;
    ets = new std::complex<T>*[nrows];
...
template<typename T> Mtx<std::complex<T>>::Mtx(int n, int m, std::complex<T> a) { // constructor
    ets = new std::complex<T>* [nrows = n] ;
    ncols = m;
    
    for (int i = 0; i< nrows; i++) {
        ets[i] = new std::complex<T>[ncols];
        for (int j = 0; j <ncols; j++) ets[i][j] = a;
    }
}

template<typename T> Mtx<std::complex<T>>::Mtx(const Mtx<std::complex<T>>& mat) { // copy constructor
    ets = new std::complex<T>* [nrows = mat.nrows];
    ncols = mat.ncols;
    for (int i = 0; i< nrows; i++) {
        ets[i] = new std::complex<T>[ncols];
        for (int j = 0; j < ncols; j++) ets[i] [j] = mat[i][j];
...
template<typename T> inline Mtx<std::complex<T>>::~Mtx(){ // destructor
    for (int i = 0; i< nrows ; i++) delete[] ets[i];
    delete[] ets;
}

template<typename T> Mtx<std::complex<T>>& Mtx<std::complex<T>>::operator=(const Mtx<std::complex<T>>& mat) { // copy assignment
    if (this != &mat) {
        if (nrows != mat.nrows || ncols != mat.ncols){ std::cerr << "\nBad matrix sizes\n"; exit(1);}
...
template<typename T> Mtx<std::complex<T>>& Mtx<std::complex<T>>::operator+=(const Mtx<std::complex<T>>& mat) { // add-assign
    if (nrows != mat.nrows || ncols != mat.ncols){
        std::cerr << "\nBad matrix sizes\n";
        exit(1);
    }
...
template<typename T> Mtx<std::complex<T>>& Mtx<std::complex<T>>::operator-=(const Mtx<std::complex<T>>& mat) { // subtract-assign
    if (nrows != mat.nrows || ncols != mat.ncols){
        std::cerr << "\nBad matrix sizes\n";
        exit(1);
    }
...
template<typename T> inline Mtx<std::complex<T>>& Mtx<std::complex<T>>::operator+() const { // usage: ml = + m2
    return *this;
}

template<typename T> inline Mtx<std::complex<T>> operator-(const Mtx<std::complex<T>>& mat)  { // usage: ml = -m2
    return (Mtx<T>(mat.nrows, mat.ncols) - mat);
}

template<typename T> Mtx<std::complex<T>> Mtx<std::complex<T>>::operator+(const Mtx<std::complex<T>>& mat) const { // m = m1 + m2
    if (nrows != mat.nrows || ncols != mat.ncols){
        std::cerr << "\nBad matrix sizes\n";
        exit(1);
    }
...
template<typename T> Mtx<std::complex<T>> operator-(const Mtx<std::complex<T>>& m1, const Mtx<std::complex<T>>& m2) { // m = m1 - m2
    if(m1.nrows != m2.nrows || m1.ncols != m2.ncols){
        std::cerr << "\nBad matrix sizes\n";
        exit(1);
    }
...
template<typename T> Vcr<std::complex<T>> Mtx<std::complex<T>>::operator*(const Vcr<std::complex<T>>& v) const { // u = m*v
    if (ncols != v.size()){
        std::cerr << "\nMatrix and vector sizes do not match\n";
        exit(1);
    }
    
    Vcr<std::complex<T>> tm(nrows);
    for (int i = 0; i < nrows; i++)
        for (int j = 0; j < ncols; j++)
            tm[i] += ets[i][j]*v[j];
...
template<typename T> std::ostream& operator<<(std::ostream& s, const Mtx<std::complex<T>>& v) {                 // output stream
    for (int i = 0; i < v.nrows; i++){
        s <<  " |";
        for (int j = 0; j < v.ncols; j++){
            s.precision(2);
...
template<typename T> int Mtx<std::complex<T>>::CG(Vcr<std::complex<T>>& x, const Vcr<std::complex<T>>& b, double& eps, int& iter) {
    if (nrows != b.size()){
        std::cout << "\nMatrix and vector sizes do not match\n";
        exit(1);
    }
    const int maxiter = iter;
    Vcr<std::complex<T>> r = b - (*this)*x;                 // initial residual
    Vcr<std::complex<T>> p = r;                             // p: search direction
    std::complex<T> zr = dot(r,r);                          // inner product of r and r
    const T stp = eps*b.twonorm();                          // stopping criterion
    ...
    for (iter = 0; iter < maxiter; iter++) {                // main loop
        Vcr<std::complex<T>> mp = (*this)*p;                // matrix-vector multiply
        std::complex<T> alpha = zr/dot(mp,p);               // divisor=0 only if r = 0
        x += alpha*p;                                       // update iterative soln
        r -= alpha*mp;                                      // update residual
        if (r.twonorm() <= stp) break;                      // stop if converged
        std::complex<T> zrold = zr;
        zr = dot(r,r);                                      // dot product
        p = r + (zr/zrold)*p;                               // zrold = 0 only if r = 0
    }
